...
void wxSightFrame::CreateToolbar() {
  ...
  clsAuiManager->AddPane(stateToolBar, wxAuiPaneInfo().Name(wxT("ToolBar")).Caption(wxT("ToolBar")).ToolbarPane().Top().LeftDockable(false).RightDockable(false));
} // end method
...
void wxSightFrame::SaveAs(wxCommandEvent &event) {
  wxString filename = wxFileSelector(_("File name"), _(""), _(""), _("cif"), _("*.cif"), wxFD_SAVE);
  if (!filename.empty()) {
...
void wxSightFrame::Ask(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (params[0])
    wxMessageBox(_T("DoubleBuffer Supported"), _T("Yupeee \\o/"), wxOK | wxICON_INFORMATION, this);
  else
    wxMessageBox(_T("DoubleBuffer Not Supported"), _T("HELL"), wxOK | wxICON_INFORMATION, this);
