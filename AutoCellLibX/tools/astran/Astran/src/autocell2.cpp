...
bool AutoCell::testGap(vector<TransitorTerminal>::iterator last_it, vector<TransitorTerminal>::iterator eulerPath_it, vector<TransitorTerminal> &ordering, TransTerminalType type) {
  ...
  if (eulerPath_it != ordering.begin() && eulerPath_it->link != -1 && type != GAP) {
    if ((last_it->type == SOURCE && ((eulerPath_it->type == SOURCE && currentNetList.getTrans(last_it->link).drain != currentNetList.getTrans(eulerPath_it->link).source) ||
                                     (eulerPath_it->type == DRAIN && currentNetList.getTrans(last_it->link).drain != currentNetList.getTrans(eulerPath_it->link).drain))) ||
        (last_it->type == DRAIN && ((eulerPath_it->type == SOURCE && currentNetList.getTrans(last_it->link).source != currentNetList.getTrans(eulerPath_it->link).source) ||
...
void AutoCell::route(bool hPoly, bool increaseIntTracks, int reduceVRt, bool optimize) {
  ...
  for (int x = 0; x < currentNetList.getOrderingN().size(); x++) {
    ...
    for (p = center - 1; p && (trackPos[p] > nDif_iniY); --p)
      ;
    for (; p && increaseIntTracks && p > 1 && trackPos[p] > nDif_endY + currentRules->getIntValue(currentNetList.getTrans(currentNetList.getOrderingN()[x].link).width) && trackPos[p] > supWidth + currentRules->getRule(S2M1M1) + currentRules->getRule(S2M1M1); --p)
      ;
  ...
  while (eulerPathP_it != currentNetList.getOrderingP().end() && eulerPathN_it != currentNetList.getOrderingN().end()) {
    ...
    while (nDiffTrackEnd && (trackPos[nDiffTrackEnd - 1] >= trackPos[nDiffTrackIni] - currentRules->getIntValue(currentNetList.getTrans(eulerPathN_it->link).width)))
      nDiffTrackEnd--;
    while (pDiffTrackEnd < trackPos.size() - 1 && (trackPos[pDiffTrackEnd + 1] <= trackPos[pDiffTrackIni] + currentRules->getIntValue(currentNetList.getTrans(eulerPathP_it->link).width)))
      pDiffTrackEnd++;
          ...
          for (int c = 0; c < trackPos.size(); c++) {
            ...
            if (befLastPolNodes[c] != "") {
              if (experimental && c < x)
                ...
                insertDistanceRuleDumb(geometries, cpt, befLastPolNodes[c], currentPolNodes[x], currentRules->getRule(S1P1P1), H, POLY);
            }
          }
          for (int c = x - 1; c >= 0; --c) {
...
string AutoCell::newDif(vector<Box *> &geometries, Compaction &cpt, string &lastGate, string &lastDiff, string &currentDiff, string &diffEnc, layer_name l, bool endDiff) {
  ...
  if (!endDiff) {
    ...
    if (lastGate != "") {
      if (lastDiff == "")
        throw AstranError("Float source/drain is not supported by ASTRAN (net degree = 1). Check your spice netlist!");
  ...
  if (lastGate != "") {
    ...
    if (currentDiff != "")
      cpt.insertConstraint("x" + lastGate + "b", "x" + currentDiff + "a", CP_MIN, "b" + lastDiff + "_LshapeAfterGate", currentRules->getRule(S1DFP1));
...
void AutoCell::createNode(vector<Box *> &geometries, Compaction &cpt, list<Element>::iterator elements_it, int pos, vector<string> &currentNode, string netName, layer_name l) {
  ...
  cpt.insertConstraint("ZERO", "b" + currentGeo + "_endline_v" + " + "
                                                                 "b" +
